<?php
	...
	$checkQuery = $conn->prepare('SELECT COUNT(id) as personnelCount FROM personnel WHERE departmentID = ?');
	...
	$data = [];
	$data["name"] = $row["name"];
	$data["personnelCount"] = $personnel['personnelCount'];

	// If the department is used in personnel, return an error
	if ($personnel['personnelCount'] > 0) {
		$output['status']['code'] = "409"; 
		$output['status']['name'] = "conflict";
		$output['status']['description'] = "Cannot delete department because it is being used by " . $personnel['personnelCount'] . " personnel(s)";
		$output['status']['returnedIn'] = (microtime(true) - $executionStartTime) / 1000 . " ms";
		$output['data'] = $data;
