<?php
	...
	// If the location is used in departments, return an error
	if ($row['count'] > 0) {
		$output['status']['code'] = "409"; 
		$output['status']['name'] = "conflict";
		$output['status']['description'] = "Cannot delete location because it is being used by " . $row['count'] . " department(s)";
		$output['status']['returnedIn'] = (microtime(true) - $executionStartTime) / 1000 . " ms";
		$output['data'] = [];
	...
	// If the location is not used in departments, proceed to delete it
	$deleteQuery = $conn->prepare('DELETE FROM location WHERE id = ?');
	$deleteQuery->bind_param("i", $_GET['id']);
	$deleteQuery->execute();
	
	if (false === $deleteQuery) {
		$output['status']['code'] = "400";
		$output['status']['name'] = "executed";
		$output['status']['description'] = "delete query failed";	
		$output['data'] = [];
