...
// PERSONNEL SECTION
$("#personnelBtn").click(() => {getAllPersonnel(); $("#filterBtn").attr("disabled", false)});
...
// DEPARTMENT SECTION
$("#departmentsBtn").click(() => {getAllDepartments(); $("#filterBtn").attr("disabled", true)});
  ...
  makeDeleteRequest("checkDeleteDepartment", $("#deleteDepartmentID").val(), (result) => {
    if (result.status.code === "409") {
      ...
      makePostRequest("getDepartmentByID", "#deleteDepartmentModal", data, (result) => {
        console.log(result.data.department[0].name)
...
// LOCATION SECTION
$("#locationsBtn").click(() => {getAllLocations(); $("#filterBtn").attr("disabled", true)});
